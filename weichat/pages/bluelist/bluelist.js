...
Page({
  ...
  exportData: function () {
    ...
    var zqvalue = that.data.zqvalue;
    var drostate = that.data.drostate;
    var drores = that.data.drores;
    var gname = that.data.gname;
    var droname = that.data.droname;

    // 构建导出URL
    var exportUrl = 'https://xj.tajian.cc/servers/sendexcel/blueinspect.php?' +
      'tid=' + tid +
      '&hyid=' + hyid +
      '&zqvalue=' + encodeURIComponent(zqvalue) +
      '&drostate=' + drostate +
      '&drores=' + drores +
      '&gname=' + encodeURIComponent(gname) +
      '&droname=' + encodeURIComponent(droname);
    ...
    wx.setClipboardData({
      ...
      success: function () {
        wx.showModal({
          title: '导出提示',
          content: '导出链接已复制到剪贴板，请在浏览器中打开下载Excel文件',
          showCancel: false,
          confirmText: '知道了'
        });
      }
