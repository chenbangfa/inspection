<?php 
	...
	//未巡检统计
	$results = $db->getAll("blueInspect","patrolCycle='每年'","addtime desc");
	foreach($results as $odres)
	{
		...
		$addTime=$row["addTime"];
				...
				$startTime = date('Y-m-d H:i:s',mktime(0,0,0,1,1,date('Y',strtotime("-1 year"))));
            	$endTime = date('Y-m-d H:i:s',mktime(23,59,59,12,31,date('Y',strtotime("-1 year"))));
				if($addTime<$endTime)
				{
